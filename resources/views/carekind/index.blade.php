...
<header>
    <div class="header-left">
        <h1>{{ $patient->pt_name ?? '患者名不明' }} のケア情報</h1>
    </div>
...
<main>
    ...
    @if ($patient)
        @forelse ($patient->ptSchedules as $schedule)
            @php
                $careList = $schedule->carekind ?? collect();
            @endphp

            @if ($careList->isEmpty())
                <div class="no-data">ケアなし（スケジュールID: {{ $schedule->pt_schedule_id }}）</div>
            @else
                <div class="no-data">スケジュールID: {{ $schedule->pt_schedule_id }}</div>
                <ul>
                    @foreach ($careList as $care)
                        @if ($care->carekindMaster)
                            <li class="care-item">
                                ケア: {{ $care->carekindMaster->value ?? '未設定' }}
                            </li>
                        @endif
                    @endforeach
                </ul>
